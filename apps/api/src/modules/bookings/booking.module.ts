...
@Module({ controllers: [BookingController], providers: [{ provide: BookingServiceImpl, useFactory: () => { if (process.env.NODE_ENV !== 'test' && !process.env.DATABASE_URL) throw new Error('DATABASE_URL is required for booking execution'); return new BookingServiceImpl(new InMemoryBookingStore(), new MockOrderService()); } }], exports: [BookingServiceImpl] })
