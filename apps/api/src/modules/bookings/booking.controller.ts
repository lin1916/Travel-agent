...
export class BookingController {
  ...
  async create(@Headers('x-actor-id') actorId: string, @Req() request: any, @Param('tripId') tripId: string, @Body() body: any) {
    ...
    return this.service.create(actorId, { ...parsed.data, tripId, offerKind: parsed.data.offerKind as any, requestId: context.requestId, correlationId: context.correlationId });
  }

  @Post('booking-intents/:intentId/commit')
  @HttpCode(HttpStatus.ACCEPTED)
  async commit(@Headers('x-actor-id') actorId: string, @Req() request: any, @Param('intentId') intentId: string, @Body() body: any) {
    if (!actorId) throw new ApplicationError('unauthorized');
    const parsed = CommitSchema.safeParse(body);
    if (!parsed.success) throw new ApplicationError('validation_error', parsed.error.issues[0]?.message);
    const context = ensureCorrelationContext(request);
    return this.service.commit(actorId, { ...parsed.data, intentId, requestId: context.requestId, correlationId: context.correlationId });
  }
