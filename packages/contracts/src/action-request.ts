...
export interface ActionRequestView extends ActionRequestInput { id: string; status: ActionRequestStatus; reasons: PolicyReason[]; expiresAt: string; version: number; requestId?: string }
...
export const ActionRequestViewSchema = ActionRequestInputSchema.extend({ id: z.string().min(1), status: ActionRequestStatusSchema, reasons: z.array(PolicyReasonSchema), expiresAt: z.string(), version: z.number().int().nonnegative(), requestId: z.string().min(1).optional() });
