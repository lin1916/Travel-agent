import { z } from 'zod';
export const AppErrorCodeSchema = z.enum(['validation_error','unauthorized','forbidden','conflict','gone','policy_blocked','supplier_unavailable','unknown_external_result']);
...
export function toPublicError(error: AppError): Omit<AppError, 'internalDetail'> {
  ...
  const correlationId = /^[A-Za-z0-9._:-]{1,128}$/.test(error.correlationId) && !/(?:\d{7,}|[\w.+-]+@[\w.-]+\.[A-Za-z]{2,})/.test(error.correlationId) ? error.correlationId : '[REDACTED_ID]';
  return { code: error.code, httpStatus: safe.httpStatus, retryable: safe.retryable, publicMessage: safe.publicMessage, correlationId };
}
const defaults: Record<AppErrorCode, Pick<AppError, 'httpStatus'|'retryable'|'publicMessage'>> = {
  ...
  gone: { httpStatus: 410, retryable: false, publicMessage: 'The requested resource is no longer available.' },
