...
export function auditEntryId(entry: Pick<AuditEntry, 'actorId' | 'action' | 'resource' | 'requestId' | 'correlationId'> & Partial<Pick<AuditEntry, 'policyResult' | 'reason' | 'occurredAt'>>): string {
  return createHash('sha256')
    .update([entry.actorId, entry.action, entry.resource, entry.requestId, entry.correlationId, entry.policyResult ?? '', entry.reason ?? '', entry.occurredAt ?? ''].join('\u0000'))
    .digest('hex');
}

export class AuditRepository {
  constructor(private readonly db: Kysely<Database>) {}
  async append(entry: AuditEntry & { tripId?: string; supplierId?: string; allowedFields?: string[] }, tx?: DatabaseTransaction): Promise<void> {
    const connection: any = tx ?? this.db;
    await connection.insertInto('audit_entries').values({ id: auditEntryId(entry), trip_id: entry.tripId ?? null, actor_id: entry.actorId, action: entry.action, resource: entry.resource, supplier_id: entry.supplierId ?? null, allowed_fields_json: entry.allowedFields ? JSON.stringify(entry.allowedFields) : null, policy_result: entry.policyResult, reason: entry.reason ?? null, mandate_version: entry.mandateVersion ?? null, grant_ref: entry.grantRef ?? null, request_id: entry.requestId, correlation_id: entry.correlationId, occurred_at: entry.occurredAt, created_at: new Date().toISOString() }).onConflict((oc: any) => oc.column('id').doNothing()).execute();
  }
