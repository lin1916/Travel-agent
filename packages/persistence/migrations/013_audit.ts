...
export async function down(db: Kysely<Database>): Promise<void> {
  await db.schema.dropTable('audit_entries').ifExists().execute();
  await sql`alter table agent_runs drop column if exists correlation_id`.execute(db);
}
