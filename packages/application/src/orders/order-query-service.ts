...
export interface OrderQueryStore { listByTrip?(tripId: string): Promise<OrderProjection[]> }
export class OrderQueryService { constructor(private readonly store: OrderQueryStore | BookingRepository) {} async listByTrip(tripId: string, _actorId: string): Promise<OrderProjection[]> { return this.store.listByTrip ? this.store.listByTrip(tripId) : []; } }
