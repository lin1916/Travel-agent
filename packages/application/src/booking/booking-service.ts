...
import type { ActionRequestInput, ActionRequestView, BookingIntentStatus, ItineraryItem, PolicySnapshot, RevalidationResult, SupplierOrderSnapshot, TravelMandate } from '@travel/contracts';
...
export interface BookingIntentStore { create(intent: BookingIntentAggregate & { [key: string]: unknown }): Promise<void>; get(id: string): Promise<(BookingIntentAggregate & { [key: string]: unknown }) | null>; save(intent: BookingIntentAggregate & { [key: string]: unknown }): Promise<void>; saveSupplierOrder?(order: PersistedSupplierOrder): Promise<void>; getSupplierOrder?(id: string): Promise<{ ownerId: string; snapshot: SupplierOrderSnapshot } | null>; getSupplierOrderForIntent?(intentId: string): Promise<{ ownerId: string; supplierId: string; snapshot: SupplierOrderSnapshot } | null>; claimCommit?(actorId: string, idempotencyKey: string, request: unknown): Promise<'claimed' | 'replay' | 'conflict'>; getCommitResult?(actorId: string, idempotencyKey: string): Promise<CommitBookingResult | null>; saveCommitResult?(actorId: string, idempotencyKey: string, result: CommitBookingResult): Promise<void>; persistCommit?(intent: BookingIntentAggregate & { [key: string]: unknown }, order: PersistedSupplierOrder, result: CommitBookingResult, actorId: string, idempotencyKey: string): Promise<void> }
...
export interface BookingAuditSink { append(entry: { actorId: string; intentId: string; actionRequestId: string; mandateId: string; event: string; redactedPayload: Record<string, unknown> }): Promise<void> }
...
export class RecordingBookingAuditSink implements BookingAuditSink {
  readonly entries: Array<{ actorId: string; intentId: string; actionRequestId: string; mandateId: string; event: string; redactedPayload: Record<string, unknown> }> = [];
  async append(entry: { actorId: string; intentId: string; actionRequestId: string; mandateId: string; event: string; redactedPayload: Record<string, unknown> }): Promise<void> { this.entries.push(structuredClone(entry)); }
}
...
export class GovernedBookingAuthorization implements BookingAuthorization {
  ...
  async authorize(actorId: string, intent: BookingIntentAggregate, input: CommitBookingIntent): Promise<{ consume(): Promise<void> }> {
    ...
    const command: ActionRequestInput = { tripId: action.tripId, resourceId: action.resourceId, kind: action.kind, risk: action.risk, requestedAmount: action.requestedAmount as ActionRequestInput['requestedAmount'], supplierId: action.supplierId, bookingType: action.bookingType, refundable: action.refundable, offerSnapshotHash: action.offerSnapshotHash, requestedSensitiveFields: action.requestedSensitiveFields };
    ...
    const candidate = intent.startsAt && intent.endsAt ? { id: intent.id, version: intent.version, tripId: intent.tripId, category: intent.offerKind === 'train' || intent.offerKind === 'flight' ? 'transport' : intent.offerKind, startsAt: intent.startsAt, endsAt: intent.endsAt, confirmed: true } as ItineraryItem : undefined;
    if (candidate && findDirectOverlaps(current.itinerary ?? [], candidate).length > 0) throw new Error('booking overlaps a confirmed itinerary item');
    ...
    return { consume: async () => {
      ...
      await this.transaction(async () => {
        ...
        await this.audit.append({ actorId, intentId: intent.id, actionRequestId: action.id, mandateId: mandate.id, event: 'BookingAuthorizationConsumed', redactedPayload: { intentVersion: intent.version, supplierId: intent.supplierId, offerId: intent.offerId } });
